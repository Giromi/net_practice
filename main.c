...
/*   Updated: 2022/11/02 01:03:05 by minsuki2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned char	*my_putip_bin(unsigned char *const int_bit, int ea) // no space
{
	/*
	 0  1  2  3  4  5  6  7  8
...
void	print_out_ip2bin(void)
{
	int				ea = 0;
	unsigned char	int_bit[36]; //	32 + 32/8 - 1 + 1 = 32 + 4 = 36 or 40
	char	str[17];

	do {
		printf("변환할 ip주소를 적어주세요(X.X.X.X or /X) or (-1) : ");
		scanf(" %[^\n]", str);
		str[16] = '\0';
		if (strlen(str) > 15) {
